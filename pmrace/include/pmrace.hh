...
public:
    ...
    addr_t print_look_up_write_addr_IP_mapping(trace_entry_t* op_ptr, addr_t addr, size_t size, FILE* file);

    bool lookup_checked_addr(addr_t addr, size_t size);
    void insert_checked_addr(addr_t addr, size_t size);
    // Read debug output
    void printInconsistentReadDebug(trace_entry_t* cur_trace);
