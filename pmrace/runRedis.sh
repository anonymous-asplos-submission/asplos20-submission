...
for i in ${TESTSIZES[@]} ; do
	...
	wait
 	...
 	echo "POST_FAILURE_COMMAND $TEST_ROOT/redis-nvml/src/redis-server $TEST_ROOT/redis-nvml/redis.conf --port 6380 pmfile $PMIMAGE 8mb & (sleep 5 ; ./redis_test.sh $i 8) ; wait" >> $CONFIG_FILE
 	# Run realworkload
 	# Start PMRace
