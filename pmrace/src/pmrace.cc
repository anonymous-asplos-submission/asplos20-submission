...
bool ShadowPM::is_consistent(trace_entry_t* op_ptr, addr_t addr, size_t size)
{
    ...
    // TODO, check this.
    // This may cause some problem if the reader reads untouched consistent data.
    for (auto &it: MAP_LOOKUP(pm_status, addr, size)) {
...
void ShadowPM::update_commitVar_timestamp(){
    cerr << "Updating commit Var with timestamp " << global_timestamp << endl;
...
addr_t ShadowPM::print_look_up_write_addr_IP_mapping(trace_entry_t* op_ptr, addr_t addr, size_t size, FILE* file)
{
...
void ShadowPM::printInconsistentReadDebug(trace_entry_t* cur_trace){
    ...
    fprintf(stderr, "Associated Write");
    this->print_look_up_write_addr_IP_mapping(cur_trace, src_addr, size, stderr);
                        ...
                        if(isWriteOriginFound){
                            if(shadow_mem->lookup_checked_addr(src_addr, size)){
                                ...
                                if(!isCorrect){
                                    ...
                                    shadow_mem->printInconsistentReadDebug(cur_trace);
                                }
                            }
                        }
                    }
